...
import hiff.hiff.behiff.domain.matching.util.similarityStrategy.HobbySimilarityStrategy;
import hiff.hiff.behiff.domain.matching.util.similarityStrategy.IncomeSimilarityStrategy;
import hiff.hiff.behiff.domain.matching.util.similarityStrategy.LifeStyleSimilarityStrategy;
import hiff.hiff.behiff.domain.matching.util.similarityStrategy.MbtiSimilarityStrategy;
import hiff.hiff.behiff.domain.matching.util.similarityStrategy.SimilarityStrategy;
...
public class SimilarityFactory {
    ...
    public int getHobbySimilarity(User matcher, User matched) {
        ...
        for(UserHobby matcherHobby : matcherHobbies) {
            for(UserHobby matchedHobby : matchedHobbies) {
                String key = HOBBY_PREFIX + matcherHobby.getHobbyId() + "_" + matchedHobby.getHobbyId();
    ...
    public int getIncomeSimilarity(User matcher, User matched) {
        String key = INCOME_PREFIX + matcher.getIncome().getStartValue() + "_" + matched.getIncome().getStartValue();
    ...
    public int getLifeStyleSimilarity(User matcher, User matched) {
        ...
        List<UserLifeStyle> matchedLifeStyles = userLifeStyleRepository.findByUserId(matched.getId());
